...
### 4. permitall() URL에 따른 헤더에 Access 토큰을 담아서 보냈을 때 예외가 발생시키지 않도록 하기 위한 처리 
...
### 5. Security에 커스텀 필터인 JwtAuthenticationFilter를 등록하고 사용하는데 1번의 request가 해당 필터를 자꾸 2번 실행하는 에러 (필터 중복 등록 에러)
...
### 6. userRepository의 save메서드에서 duplicate key 에러가 발생했는데, 응답이 CustomAuthenticationEntryPoint의 코드로 처리 되는 버그
